...
- [x] Phase 4 (Teil 2): Malware-Scan für Uploads (`src/server/services/malware-scan.ts`, ClamAV, fail-closed, hinter `MALWARE_SCAN_ENABLED`); EICAR erkennt ClamAV nur am Dateianfang, eigene Testsignatur genutzt (siehe `docs/PRIVACY_SECURITY.md`). War einmal real in CI grün, dann durch eine eigene CI-Änderung (fehlender Server-Neustart nach Zwischen-Reset, D-079) zweimal fehlgeschlagen – Root-Cause per Server-Log gefunden und behoben, zweimal lokal CI-äquivalent bestätigt (8/8 grün); echter CI-Lauf nach diesem Fix noch zu prüfen.
