...
- [x] Phase 2: Android-Emulator lokal eingerichtet (Command-Line-Tools + OpenJDK 17, kostenlos, kein Konto), AVD mit Android 16/API 36 gebootet, `expo run:android` echt gebaut und installiert, per Dev-Login-Weg echt angemeldet mit echten Backend-Daten in Hell/Dunkel geprüft (D-084). Erweiterte iOS-Matrix: iPhone Air + iPad Air 11" zusätzlich zum iPhone 17 Pro, Dark Mode, größte Dynamic-Type-Stufe, `supportsTablet: false` bestätigt.
- [x] Phase 2 (Nachtrag, 25.07.2026): Toms macOS-Bedienungshilfen-Berechtigung erteilt – erster echter Tap-Durchlauf im iOS-Simulator möglich. Registrierung, Login, Einladungscode, Praxisverbindung, Telefonnummer ändern, Profilbild hochladen/ersetzen/entfernen, Passwort-/E-Mail-Änderung, Übung öffnen + Dokumentation (echter DB-Eintrag), Terminabsage mit Grund (echter DB-Eintrag), Terminangebot annehmen, Konto löschen – alle per echtem Tap verifiziert (nicht nur per Deep-Link/Code-Review wie zuvor). Dabei vier echte, vorher nie erreichbare Fehler gefunden und behoben: mehrdeutige DB-Beziehung beim Übungsdetail (D-089), kaputter Profilbild-Upload durch einen RN-0.86-Multipart-Bug (D-090), abgeschnittener Text bei größter Systemschrift (D-091/D-092). Zusätzlich: Profilbild oben rechts antippen führt jetzt direkt zu Profil (vorher kein Tap-Handler). VoiceOver-Screenreader-Test bewusst nicht durchgeführt (Toms Entscheidung); Reduce Motion und Hell/Dunkel-Systemumschaltung geprüft und unauffällig.
...
- [x] Finale volle Testmatrix nach Dokumentationsabschluss erneut bestätigt (Typecheck/Lint/115 Tests/125 RLS-Proben/E2E 55:0:23/Build/mobile grün); Draft-PR #5 aktuell gehalten.
- [x] Liquid-Glass-Design nach echtem Desktop-/Tablet-Browserdurchlauf konsistent auf allen vorgesehenen Flächen angewendet (`/practice/settings` und die Nachrichten-Liste hatten es noch gefehlt) und ein falsch beschriftetes Feld im Betreiberportal behoben (Commit `379efc3`).
- [x] Nachrichtenfunktion von Tom selbst im iOS-Simulator und Android-Emulator vollständig getestet und bestätigt („ja passt alles!").
- [x] Ausprobiert und bewusst wieder verworfen: `liquid-glass-react` (von Tom benannte Bibliothek) für die Nachrichten-Sendeschaltflächen – die Bibliothek zentriert sich selbst per `position:relative + top/left:50% + translate(-50%,-50%)` und ist für kleine, freistehende Dock-Icon-artige Elemente gebaut, nicht für Schaltflächen in normalem Formular-Flow; nach zwei echten Fixversuchen (Tailwind-`@source` für die node_modules-Klassen, dann ein Wechsel weg vom nativen `<button>`-Wrapper) blieb ein Positionierungsfehler bestehen. Vollständig zurückgerollt (Paket deinstalliert, Komponente entfernt) statt etwas sichtbar Kaputtes zu committen.
- [x] GitHub CI: alle 4 Jobs grün auf `main` nach dem Merge. Der zeitweise „Sicherheit"-Fehlschlag war eine externe npm-Registry-Störung (`npm/cli#9804`, bestätigt registry-seitig per direktem `curl`/`fetch`-Vergleich) und hat sich von selbst erledigt; ein zwischenzeitlicher pnpm-Versionssprung (11.11.0→11.17.0) brach kurz alle vier Jobs (`pnpm/action-setup` lehnt doppelt gesetzte Versionsangaben ab) und wurde sofort behoben (D-100/D-101).
- [x] PR #5 auf „Ready for review" gesetzt und mit Toms Freigabe nach `main` gemergt (`be94192`, 26.07.2026, regulärer Merge-Commit). CI auf `main` real verifiziert (grün). Obsidian-Vault final synchronisiert.

## Auftrag vom 26.07.2026 – Native Liquid-Glass-Tab-Leiste (Branch `claude/mobile-liquid-glass-tabbar-20260726`)

Auf Toms Wunsch: die untere Navigation der nativen Patienten-App soll wie eine echte Apple-iOS-26-Liquid-Glass-Tab-Leiste aussehen/sich anfühlen, nicht nur wie eine weiße schwebende Fläche mit flachem Kreis.

- [x] `@callstack/liquid-glass` installiert und in `glass-card.tsx` (Nachrichten-Screen) sowie neu in `tab-bar.tsx` eingesetzt; nativer Rebuild (`expo run:ios`) durchgeführt.
- [x] Tab-Leiste zur schwebenden, randabgesetzten Kapsel umgebaut (`position: absolute`, volle Kapselrundung) statt randlosem, im Layout eingebundenem Balken – Bildschirminhalte scrollen jetzt wirklich dahinter durch (Voraussetzung dafür, dass der Glaseffekt überhaupt sichtbar etwas zu brechen hat).
- [x] Eine einzelne, durchgehend animierte Hervorhebung (statt Pille pro Tab) gleitet zwischen den Zielen: sanfte Feder-Animation beim Antippen, direktes Folgen der Fingerposition beim Ziehen über die Leiste (ohne loszulassen).
- [x] Aktives Icon wächst animiert beim Tab-Wechsel.
- [x] Reihe von Fehlern in mehreren Iterationen gefunden und behoben: veralteter Metro-Bundler-Prozess servierte einen Stand von vor den Korrekturen; deckender statt transluzenter Tint übermalte den Glaseffekt; Fettschrift beim aktiven Label ließ „Nachrichten" umbrechen und am Kapselrand abschneiden; PanResponder-Bubble-Phase ließ das Ziehen nie auslösen (Kind-`<Pressable>` beanspruchte die Geste zuerst); eigener Rechenfehler positionierte die Hervorhebung am linken Rand statt zentriert.
- [x] `unstable-native-tabs` (Expo Router) und `expo-glass-effect` (offizielles Expo-Pendant) geprüft, bewusst nicht eingesetzt (D-106/D-107, Gründe dort).
- [x] Ehrlich dokumentiert: echte chromatische Dispersion/Lichtbrechung mit Farbsäumen ist über öffentliche iOS-APIs nicht erreichbar (D-108) – die technisch bestmögliche Annäherung (nativer Blur/Lensing + echte Bewegung) ist umgesetzt.
- [x] Typecheck/Lint/16 mobile Tests grün nach jeder Änderung; zwei gezielte, kommentierte `eslint-disable`-Zeilen für Reacts neue, für PanResponder/Animated.Value zu strenge `react-hooks/refs`-Regel (D-111).
- [x] **Nachtrag (Toms 1. Rückmeldung):** „im Simulator kaum ein sichtbarer Unterschied, Schimmer-/Reflexions-/Wassertropfen-Effekt fehlt praktisch vollständig" – Tom hat der ersten Fassung (reine native Glasfläche, D-108/D-109) ausdrücklich NICHT zugestimmt. Klargestellt: die API-Grenze (D-108) ist keine Abschlussbegründung, sondern nur der Hinweis, dass echte Dispersion nicht erreichbar ist – eine sichtbare, simulierte Annäherung wird trotzdem verlangt.
- [x] **D-112: Eigener optischer Effekt-Layer über der nativen Glasfläche** (`liquid-lens.tsx`, neue Komponente): Haupt-Reflexionsbogen oben links, subtilerer Gegenreflex unten rechts, cyanfarbene Kante oben / magentafarbene Kante unten (simulierte Farbsäume, nie ein voll eingefärbter Kreis), räumlicher Rand, weicher Schatten, wandernder Schimmer beim Tab-Wechsel. Alles `pointerEvents="none"`, beeinflusst nie Touch/Ziehen/Navigation. Neue Abhängigkeit `expo-linear-gradient` (Version exakt passend zum installierten Expo SDK 57.0.1), nativer Rebuild durchgeführt.
