...
- [x] Phase 4 (Teil 2): Malware-Scan für Uploads (`src/server/services/malware-scan.ts`, ClamAV, fail-closed, hinter `MALWARE_SCAN_ENABLED`); lokal UND in echtem CI-Lauf verifiziert (EICAR erkennt ClamAV nur am Dateianfang, eigene Testsignatur genutzt, siehe `docs/PRIVACY_SECURITY.md`); erster CI-Lauf schlug an einem zu knappen Zeitlimit fehl, nach Fix grün (`gh run view 29854798546`).
