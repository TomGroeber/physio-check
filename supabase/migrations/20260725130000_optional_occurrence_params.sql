-- ============================================================
-- Nachtrag (unabhaengig vom Betreiber-Auftrag, beim Regenerieren
-- von database.types.ts entdeckt): record_exercise_occurrence
-- deklarierte p_sets_completed/p_pain_before/p_pain_after als
-- nicht-optionale int-Parameter, obwohl Selbstauskunft-Werte
-- laut docs/PRODUCT_SPEC.md F6 ausdruecklich optional sind
-- ("optional Schmerz vor/nach"). Postgres liess NULL an dieser
-- Stelle immer durch (kein NOT NULL im Funktionskopf) - die
-- fehlenden 'default null' fuehrten aber dazu, dass
-- `supabase gen types` die Parameter faelschlich als
-- nicht-nullable typisierte, was den Typecheck fuer die
-- bestehenden Aufrufer (exercise-log.ts, rls-tests.ts) brach,
-- sobald die Typen neu erzeugt wurden. Funktionskoerper
-- unveraendert, nur die Parameterzeile ergaenzt.
-- ============================================================
