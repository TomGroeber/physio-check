...
## Ergänzung 2026-07-25/26: Betreiberportal, Liquid-Glass-Design, Nachrichtenfunktion

**Betreiberportal:** Eine neue `platform_admins`-Tabelle (keine Client-Policies) markiert Plattform-Admins, strikt getrennt von `practice_members` – die Rolle ist über keinen Client-Weg herleitbar oder selbst zuweisbar, nur über das idempotente CLI-Skript `scripts/platform-admin.ts` (Service Role). Jede `/admin`-Route und -Aktion ruft zuerst `assertPlatformAdmin()` auf. Cross-Tenant-Lesezugriffe (Praxislisten/-details für den Betreiber) laufen über den Service-Client, aber immer NACH dieser Prüfung; Schreibzugriffe laufen über SECURITY-DEFINER-RPCs auf der eigenen Sitzung des Admins (`create_practice_with_admin_invite`, `set_practice_lifecycle`, `update_practice_settings`, …) – dasselbe Muster wie die bestehenden Patienteneinladungen, nicht ein neuer, pauschaler Service-Role-Zugriff. `staff_invites` erweitert das hash-only Einladungsmuster von `patient_invites` auf Mitarbeitende: atomare Annahme bindet das verifizierte Konto per E-Mail-Abgleich exakt an Praxis+Rolle. Praxis-Lebenszyklus (`trial/active/suspended/archived`) ist spaltenbasiert mit Trigger-Schutz (`prevent_practice_lifecycle_self_edit`) gegen versehentliche Selbstbearbeitung außerhalb des Portals.

**Liquid-Glass-Design:** Rein additive OKLCH-Tokens (`--glass-*`) in `globals.css` plus eine kleine `GlassPanel`-Komponente, mit `@supports`-Fallback auf eine deckende Fläche für Browser ohne `backdrop-filter`. Bewusst kein neues UI-Framework – nur Tailwind-Utilities. Eingesetzt im Betreiberportal, den Praxis-Einstellungen und der Nachrichtenoberfläche; der übrige, ruhige Patientenbereich bleibt unverändert.

**Nachrichtenfunktion:** `conversations` (eine Zeile je Praxis+Patient:in) und `messages` (nach dem Senden unveränderlich, keine Client-Update/Delete/Insert-Policy). Alle Schreibzugriffe laufen über SECURITY-DEFINER-RPCs, die jede ID serverseitig herleiten: `send_patient_message()` nimmt keine Praxis-ID entgegen, sondern liest die aktive `patient_practice_links`-Zeile der aufrufenden Person; `send_practice_reply()` prüft Praxismitgliedschaft UND dass die Patientin/der Patient aktuell noch verbunden ist. Anders als beim `completion_logs`-Muster (Praxis behält Lesezugriff auf eigene Historie unabhängig vom Link-Status) verliert eine Praxis hier nach einem Praxiswechsel sowohl Schreib- als auch Leserecht auf die Unterhaltung – die Patientin behält ihren eigenen Lesezugriff immer (`patient_profile_id = auth.uid()`, kein Link-Status nötig). Kein Supabase Realtime (wäre die erste Nutzung im Projekt gewesen und hätte eine eigene Autorisierungsprüfung gebraucht); stattdessen sichtbarkeitsbasiertes Polling wie an anderer Stelle im Projekt (`revalidatePath` + periodisches `router.refresh()`).
