...
> Stand: 2026-08-02 · `main` (PR #9–#18: großer Review-Auftrag Phasen A–N) ist gemergt. **Aktuell IN ARBEIT (nicht gemergt):** Branch `claude/illustrated-docs-20260802` – Phase O (bebilderte GitHub-Doku) abgeschlossen, noch nicht committet/gepusht/gemergt. GitHub-Remote: `TomGroeber/physio-check` (öffentlich, D-036; keine Secrets/echten Daten)
...
**Stand:** A, B/C, D, F, E, H, I, J, K, M, N abgeschlossen und gemergt (PR #9–#18). Phase N: Tom hat die macOS-Bedienungshilfen-Berechtigung erteilt, die bisher jede Tap-Interaktion im Simulator blockierte (D-151) – erster echter Simulatorlauf mit echten Taps durchgeführt, Tom tippt selbst und schickt Screenshots statt token-teurer `cliclick`-Automatisierung durch die KI. Kernflüsse alle grün (D-153); Profilbild-Upload, Terminabsage, Terminangebote auf nativ und Android bleiben offen (Tom dokumentiert Weiteres in einem eigenen Trello-Board). Phase O: 13 echte Screenshots (kein Mockup) aus der laufenden Demo-Umgebung erzeugt und in `README.md` (neuer Abschnitt „Screenshots") sowie `docs/PLATFORM_ADMIN_GUIDE.md` eingebunden (D-154/D-155) – erfüllt dabei auch ein zuvor nicht eingehaltenes Versprechen im README („…mit allen Details und Screens"). Als Nächstes: Phase P (Abschlussbericht) – letzte Phase des großen Auftrags.
