...
**3. Aktuell blockiert:** GitHub-CI-Job „Sicherheit" durch eine externe npm-Registry-Störung (`ERR_PNPM_AUDIT_BAD_RESPONSE`, s. D-099) – muss erneut laufen, sobald sich die Registry erholt. Echte Simulator-/Emulator-Verifikation der neuen Nachrichtenoberfläche führt Tom selbst durch (nicht mehr automatisiert in dieser Sitzung, auf seinen ausdrücklichen Wunsch) – App ist auf iPhone-17-Pro-Simulator und Android-Emulator (`physiocheck_test`) bereits installiert und mit Metro verbunden, Backend läuft lokal.
