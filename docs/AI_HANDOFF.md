...
> Stand: 2026-08-02 · `main` (PR #9–#17: großer Review-Auftrag Phasen A–M + CI-Zuverlässigkeit) ist gemergt. **Aktuell IN ARBEIT (nicht gemergt):** Branch `claude/native-testing-20260802` – Phase N (nativer iOS-Simulatorlauf mit echten Taps) im Kernumfang abgeschlossen, noch nicht committet/gepusht/gemergt. GitHub-Remote: `TomGroeber/physio-check` (öffentlich, D-036; keine Secrets/echten Daten)
...
**Stand:** A, B/C, D, F, E, H, I, J, K, M abgeschlossen und gemergt (PR #9–#16), CI-Zuverlässigkeitsfix ebenfalls gemergt (PR #17, D-149/D-150). Phase N: Tom hat die macOS-Bedienungshilfen-Berechtigung erteilt, die bisher jede Tap-Interaktion im Simulator blockierte (D-151) – erster echter Simulatorlauf mit echten Taps (nicht nur Deep-Link-Navigation) durchgeführt. Praktischer Ablauf dabei: **Tom tippt selbst im Simulator und schickt Screenshots, die KI wertet aus und sagt den nächsten Schritt an** – koordinatenbasierte `cliclick`-Automatisierung durch die KI selbst erwies sich als fehleranfällig und token-teuer, Tom hat das selbst vorgeschlagen. Kernflüsse alle grün (D-153); Profilbild-Upload, Terminabsage und Terminangebot-Antworten auf nativ bleiben offen – Tom dokumentiert Weiteres selbst in einem eigenen Trello-Board. Android bleibt mangels installiertem Android Studio ungetestet. Als Nächstes: Phase O (bebilderte Doku) oder P (Abschlussbericht) – mit Tom abstimmen.
