...
> Stand: 2026-08-01 · `main@ba6f726` + PR #13 (Video-Upload-Verbesserungen) + PR #14 (Schnellsuche) + PR #15 (Hilfecenter) sind gemergt. **Aktuell IN ARBEIT (nicht gemergt):** Branch `claude/full-test-matrix-20260801` – Phase M (Testlücken Terminangebote/Konfliktprüfung/DST) abgeschlossen, noch nicht committet/gepusht/gemergt. GitHub-Remote: `TomGroeber/physio-check` (öffentlich, D-036; keine Secrets/echten Daten)
...
**Stand:** A, B/C, D, F, E, H, I, J, K, M abgeschlossen (jeweils eigener Branch/PR; I, J, K bereits gemergt als PR #13/#14/#15). Phase M schließt die in Phase B/C offen gelassenen Terminszenarien: Terminangebote annehmen/ablehnen/zurückziehen, Konfliktprüfung, Sommer-/Winterzeit- und Mitternacht-Grenzfälle, jeweils als neue Playwright-Tests. Dabei einen echten Fehler gefunden und behoben: die Ablehnen-Bestätigung eines Terminangebots wurde der Patientin/dem Patienten nie angezeigt, weil `OfferResponse` sich selbst `null` zurückgab, sobald die Angebotsliste nach dem Ablehnen leer war – die Prüfung dafür berücksichtigte nur den Annehmen-, nie den Ablehnen-Zustand (D-145). Als Nächstes: Phase N (native iOS/Android-Tests) – braucht Simulatoren/Geräte, größerer Umfang als die letzten Phasen.
...
**Playwright-Parallelität – lokal 4 Worker, in CI 2 (`playwright.config.ts`: `workers: process.env.CI ? 2 : 4`).** Bei Phase J zeigten sowohl Toms Mac als auch (neu, erstmals beobachtet) der CI-Runner wiederholt `page.goto`-Timeouts/`ChunkLoadError`s über mehrere, von der jeweiligen Änderung unberührte Dateien hinweg – reproduziert lokal auch nach vollständigem Docker-/Node-Neustart und per `git stash`-Vergleich als unabhängig von Codeänderungen bestätigt. Ursache in CI: der GitHub-Actions-Runner hat nur 2 Kerne (bereits vorher im Workflow selbst dokumentiert, beim isolierten Malware-Scan-Schritt, der genau deshalb `--workers=1` nutzt) – der Haupt-E2E-Lauf lief aber weiter mit 4 Workern. Seit D-139 in CI auf 2 Worker begrenzt (lokal unverändert 4). **Wichtige Erfahrung dabei:** nicht jeder CI-Fehlschlag in diesem Umfeld ist automatisch die Ressourcenknappheit – ein zweiter, echter Fehler (D-140: Namenskollision zweier „Suchen"-Knöpfe) blieb zunächst zwischen den Ressourcen-Flakes verborgen und wurde erst durch genaues Lesen aller Fehlermeldungen gefunden. Bei lokalen Timeouts: betroffene Tests einzeln oder mit `--workers=1`/`-g "Testname"` laufen lassen (dort durchgehend grün).
