...
> Stand: 2026-08-01 · `main@ba6f726` + PR #13 (Video-Upload-Verbesserungen) + PR #14 (Schnellsuche) sind gemergt. **Aktuell IN ARBEIT (nicht gemergt):** Branch `claude/help-center-20260801` – Phase K (Hilfecenter) abgeschlossen, noch nicht committet/gepusht/gemergt. GitHub-Remote: `TomGroeber/physio-check` (öffentlich, D-036; keine Secrets/echten Daten)
...
**Stand:** A, B/C, D, F, E, H, I, J, K abgeschlossen (jeweils eigener Branch/PR; I und J bereits gemergt als PR #13/#14). Phase K baut ein Hilfecenter (`/practice/help`) für Praxismitarbeitende – Inhalte per Codeprüfung recherchiert statt angenommen (u. a. bestätigt: eine Praxis kann eine Patientenverbindung aktuell NICHT selbst lösen, das Hilfecenter behauptet das bewusst nicht), statische Inhalte nach dem Muster der bestehenden Datenschutzerklärung, eigene `<details>`-Elemente statt neuer Abhängigkeit (D-141–D-143). Als Nächstes: Phase M (volle Playwright-Matrix).
