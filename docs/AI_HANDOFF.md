...
> Stand: 2026-08-01 · `main@ba6f726` ist der letzte GEMERGTE Stand (PR #12, Architektur-Konsolidierung; PR #13, Video-Upload-Verbesserungen, ist gemergt, `AI_HANDOFF.md` hier aber noch vor dem `docs:sync` des Phase-J-Merges). **Aktuell IN ARBEIT (nicht gemergt):** Branch `claude/global-search-20260801` – Phase J (praxisweite Schnellsuche) abgeschlossen, noch nicht committet/gepusht/gemergt. GitHub-Remote: `TomGroeber/physio-check` (öffentlich, D-036; keine Secrets/echten Daten)
...
**Stand:** A, B/C, D, F, E, H, I, J abgeschlossen (jeweils eigener Branch/PR; I bereits gemergt als PR #13). Phase J baut eine praxisweite Schnellsuche (Strg/Cmd+K) für Patienten, Übungen und Bereichs-Schnellzugriffe – bewusst ohne neue `cmdk`-Abhängigkeit (eigene kleine Listbox über die vorhandene `dialog.tsx`), Umfang bewusst auf die beiden bereits durchsuchbaren Kernentitäten begrenzt (D-136–D-138). Als Nächstes: Phase K (Hilfecenter).
...
**Lokale Playwright-Läufe mit voller Parallelität (Standard: 4 Worker, `playwright.config.ts`):** auf Toms Mac zeigt die volle Suite wiederholt `page.goto`-Timeouts über mehrere, jeweils von der aktuellen Änderung unberührte Dateien – reproduziert sowohl nach vielen DB-Resets in einer langen Sitzung als auch nach einem vollständigen Docker-/Node-Neustart (`supabase stop && supabase start`, frischer `pnpm dev`). Das spricht für Ressourcenkonflikt (4 parallele Chromium-Instanzen + Turbopack-Kompilierung + Docker-Supabase-Stack gleichzeitig auf derselben Maschine), nicht für einen Code-Fehler – per `git stash`-Vergleich in Phase I bestätigt: reproduziert identisch auf unverändertem Code. Abhilfe: betroffene Tests einzeln oder mit `--workers=1`/`-g "Testname"` laufen lassen (dort durchgehend grün); CI (GitHub-Actions-Runner, andere Hardware) zeigt dieses Muster bisher nicht – dort ist die volle Suite die verlässliche Instanz.
