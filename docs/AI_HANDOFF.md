...
- **Malware-Scan-Pipeline** für Übungsmedien und Profilbilder (`src/server/services/malware-scan.ts`, ClamAV, fail-closed) hinter `MALWARE_SCAN_ENABLED` (Standard aus, da lokale Entwicklung/bisherige CI keinen Scanner hatten) – lokal vollständig verifiziert inkl. echter Ablehnung (D-071/D-072). **Erster echter CI-Lauf mit ClamAV schlug fehl** (zwei Upload-Bestätigungen überschritten das globale 15s-Zeitlimit – `clamscan` braucht unter paralleler CI-Worker-Last spürbar länger als lokal isoliert gemessen); behoben durch gezielt höhere Timeouts nur für scan-betroffene Zusicherungen. **Erneute CI-Bestätigung nach diesem Push noch zu prüfen.**
...
**Offen für die nächste Etappe:** `gh run view` zur CI-Bestätigung nach dem Timeout-Fix; danach Phase 4 Rest (Dateninventur/Store-Mappings), Phase 5 Rest (Privacy Manifest), Phase 2 (Android-Emulator), Phase 6–8.
