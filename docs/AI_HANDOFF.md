...
- **Malware-Scan-Pipeline** für Übungsmedien und Profilbilder (`src/server/services/malware-scan.ts`, ClamAV, fail-closed) hinter `MALWARE_SCAN_ENABLED` (Standard aus, da lokale Entwicklung/bisherige CI keinen Scanner hatten) – lokal UND in einem echten CI-Lauf verifiziert (D-071/D-072). Erster echter CI-Lauf mit ClamAV schlug fehl (zwei Upload-Bestätigungen überschritten das globale 15s-Zeitlimit unter paralleler CI-Worker-Last); nach gezielt höheren Timeouts für scan-betroffene Zusicherungen erneut gelaufen und **grün bestätigt** (`gh run view 29854798546`, alle 4 Jobs erfolgreich).
...
**Offen für die nächste Etappe:** Phase 5 Rest (iOS Privacy Manifest, finale App-Identität – Toms Entscheidung), Phase 2 (Android-Emulator-Ersteinrichtung), Phase 3 (Deployment-Vorbereitung ohne Kontoerstellung), Phase 6 Rest (Screenshots, blockiert durch A2/A7), Phase 7–8.
