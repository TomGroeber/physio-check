...
> Stand: 2026-08-02 · `main` (PR #9–#19: großer Review-Auftrag Phasen A–O) ist gemergt. **Aktuell IN ARBEIT (nicht gemergt):** Branch `claude/practice-handbook-20260802` – bebildertes Praxishandbuch (Toms Zusatzwunsch nach Phase O) abgeschlossen, noch nicht committet/gepusht/gemergt. GitHub-Remote: `TomGroeber/physio-check` (öffentlich, D-036; keine Secrets/echten Daten)
...
**Stand:** A, B/C, D, F, E, H, I, J, K, M, N, O abgeschlossen und gemergt (PR #9–#19). Phase N: Tom hat die macOS-Bedienungshilfen-Berechtigung erteilt, die bisher jede Tap-Interaktion im Simulator blockierte (D-151) – erster echter Simulatorlauf mit echten Taps durchgeführt, Tom tippt selbst und schickt Screenshots statt token-teurer `cliclick`-Automatisierung durch die KI. Kernflüsse alle grün (D-153); Profilbild-Upload, Terminabsage, Terminangebote auf nativ und Android bleiben offen (Tom dokumentiert Weiteres in einem eigenen Trello-Board). Phase O: 13 echte Screenshots (kein Mockup) aus der laufenden Demo-Umgebung erzeugt und in `README.md` (neuer Abschnitt „Screenshots") sowie `docs/PLATFORM_ADMIN_GUIDE.md` eingebunden (D-154/D-155) – erfüllt dabei auch ein zuvor nicht eingehaltenes Versprechen im README („…mit allen Details und Screens").

**Zusatzwunsch nach Phase O (IN ARBEIT, dieser Branch):** Tom fand die README-Screenshots gut, wollte aber zusätzlich ein vollständiges bebildertes Handbuch für Praxismitarbeitende – jede Funktion mit Screenshot UND erklärendem Text, Pfeile/Kreise im Bild wo hilfreich. Neu: `docs/PRACTICE_HANDBOOK.md` mit 16 zusätzlichen, gezielt annotierten Screenshots (`docs/screenshots/handbook-*.png`), erzeugt über dieselbe SVG-Overlay-Technik wie in Phase O plus eine neue Pfeil-Variante (`annotateArrow`, s. `D-156`). Zwei fehlerhafte Annotationen beim Prüfen gefunden und korrigiert, bevor Texte dazu geschrieben wurden (falscher Locator, vergessenes Zurücksetzen einer Annotation vor dem Scrollen – s. D-156 und TASKS.md). Als Nächstes: diesen Branch fertig verifizieren/committen/PR/merge, danach Phase P (Abschlussbericht) – letzte Phase des großen Auftrags.

**Für zukünftige native Simulator-Sitzungen:** die macOS-Bedienungshilfen-Berechtigung ist jetzt erteilt (Terminal), Docker kann bei Speicherplatzproblemen mit `docker builder prune -f` entlastet werden (D-152). Für Formularinteraktion (Tippen) im Simulator ist der Tom-gestützte Ablauf (er tippt, schickt Screenshots) dem koordinatenbasierten `cliclick` vorzuziehen – letzteres bleibt für reine, unkritische Taps (z. B. einen einzelnen bekannten Button) nutzbar, aber die Fenstervermessung (`System Events`, Gruppenposition/-größe) muss bei jeder neuen Simulator-Fensterposition neu erfolgen.
...
**Dritter bekannter, akzeptierter CI-Flake: Restvarianz auf dem geteilten 2-Kern-Runner (seit D-149/D-150 deutlich seltener, aber nicht null).** Nach dem Vorab-Kompilieren-Fix wurden testweise vier aufeinanderfolgende CI-Läufe desselben, unveränderten Standes beobachtet: einer komplett grün, andere mit vereinzelten (teils selbstheilenden) Fehlschlägen bei `help-center.spec.ts`/`global-search.spec.ts` oder – unabhängig davon – im isolierten Malware-Scan-Schritt (echter `clamscan`, bereits vorher als CPU-sensibel dokumentiert). Laufzeiten desselben Tests schwankten zwischen 4,3 und 6,9 Minuten – klassische Infrastruktur-Varianz eines geteilten 2-Kern-Runners, nicht mehr durch einen weiteren gezielten Fix zu beheben, ohne die betroffenen Tests in einen eigenen seriellen Schritt auszulagern (wie beim Malware-Scan). Bewusst akzeptiert statt weiter verfolgt – dieselbe Kategorie wie die beiden längst bekannten Flakes (`expo-doctor`-Registry-Drift, gelegentliche Supabase-Auth-Verzögerung nach `db:reset`): dokumentiert, per Rerun bestätigt als nicht code-bedingt, kein Blocker für einen Merge.
