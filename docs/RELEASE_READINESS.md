...
- ~~**`docs/PRIVACY_SECURITY.md`** ... es fehlt ... App-Privacy-/Data-Safety-/Health-Declaration-Mappings~~ – Mapping-Tabelle ergänzt 2026-07-21, siehe A4/A6. ~~Eine konsolidierte, store-taugliche Datenschutzerklärungs-Vorlage fehlt~~ – `/privacy`-Seite ergänzt 22.07.2026, siehe A4.
...
| Punkt | Status | Beleg | Nächster Schritt |
|---|---|---|---|
| iOS-Simulator-Tap-Durchlauf (voller Patientenablauf) | ERLEDIGT UND VERIFIZIERT | Tom hat macOS-Bedienungshilfen freigegeben (25.07.2026) – vollständiger echter Tap-Durchlauf: Registrierung, Login, Einladungscode, Praxisverbindung, Telefonnummer, Profilbild hoch-/ersetzen/entfernen, Passwort-/E-Mail-Änderung, Übung öffnen + Dokumentation, Terminabsage, Terminangebot annehmen, Konto löschen. Vier reale Fehler dabei gefunden und behoben (D-089 bis D-092, siehe `docs/TEST_MATRIX.md`) | – |
| Kleines + großes iPhone, Dark Mode, Dynamic Type, VoiceOver, Reduce Motion | ERLEDIGT UND VERIFIZIERT (ohne VoiceOver – Toms Entscheidung) | iPhone Air + iPad Air 11" (M4) zusätzlich zum iPhone 17 Pro geprüft. **Nachtrag 25.07.2026:** Die frühere statische Prüfung der größten Dynamic-Type-Stufe (nur Willkommensbildschirm, kein Live-Interaktionstest) hatte einen echten Clipping-Fehler übersehen, den erst Toms eigener Tap-Durchlauf mit System-Bedienungshilfen fand (D-091/D-092, jetzt behoben). Reduce Motion und System-Dunkelmodus-Umschaltung live geprüft, unauffällig | Lehre: Dynamic-Type-Prüfung ohne echte Live-Interaktion auf mehreren Screens reicht nicht aus |
...
| Öffentliche Datenschutz-/Support-/Löschseite | ERLEDIGT – RECHTLICHE PRÜFUNG AUSSTEHEND | `src/app/account-deletion/page.tsx` UND `src/app/privacy/page.tsx`, beide ohne Login erreichbar (HTTP 200 verifiziert per Playwright-Screenshot). Datenschutzerklärung ist ein ehrlicher technischer Entwurf mit deutlich sichtbarem Hinweis „noch nicht rechtlich geprüft" (CLAUDE.md-Regel: keine Rechtskonformität behaupten) – Verantwortlicher/Rechtsgrundlage/Aufbewahrungsfrist bewusst als offen markiert | Von einer für Datenschutz zuständigen Person prüfen lassen, sobald App-Identität feststeht; Support-E-Mail ist Platzhalter bis Toms Entscheidung (A5) |
