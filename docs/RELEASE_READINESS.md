...
| Malware-Scan für Uploads | ERLEDIGT UND VERIFIZIERT | `src/server/services/malware-scan.ts` (ClamAV `clamscan`, fail-closed); in `finalizeAvatarUpload`/`finalizeUpload` eingebaut hinter `MALWARE_SCAN_ENABLED`; lokal UND in einem echten CI-Lauf verifiziert (`gh run view 29854798546` – alle 4 Jobs grün, `web-database`-Job installiert ClamAV real und scannt echt). Erster echter CI-Lauf schlug wegen zu knappem Zeitlimit unter paralleler Worker-Last fehl (`clamscan` braucht dort spürbar länger als lokal isoliert); behoben durch gezielt höhere Timeouts, danach grün bestätigt | Produktions-Dauerlösung (`clamd`-Daemon oder Cloud-AV) bleibt an Toms Hosting-Wahl gebunden (siehe A3) |
