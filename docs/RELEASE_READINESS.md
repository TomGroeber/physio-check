...
| Malware-Scan für Uploads | ERLEDIGT – CI-LAUF NACH LETZTEM FIX AUSSTEHEND | `src/server/services/malware-scan.ts` (ClamAV `clamscan`, fail-closed) in `finalizeAvatarUpload`/`finalizeUpload` hinter `MALWARE_SCAN_ENABLED`. Ein Isolations-Versuch (parallele Scan-Last mit einem separaten, seriellen Lauf begegnen) führte über mehrere CI-Läufe zu drei echten, jeweils per Log root-caused Bugs: D-079 (Server-Neustart nach Zwischen-Reset fehlte), D-080 (Erkenntnis: `MALWARE_SCAN_ENABLED=true` fehlte bei manuellen Test-Serverstarts), D-081 (derselbe Fehler wie D-080, aber in der CI-YAML selbst – der Server startete ohne die Variable, nur der Playwright-Befehl hatte sie; per direkten `clamscan`-Diagnoseaufrufen und Server-Log bestätigt, nicht angenommen). Nach der Korrektur lokal CI-äquivalent reproduziert: 8/8 grün, < 52s | Nach Push echten CI-Lauf bestätigen; Produktions-Dauerlösung (`clamd`-Daemon oder Cloud-AV) bleibt an Toms Hosting-Wahl gebunden (siehe A3) |
