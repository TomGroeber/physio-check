...
| Malware-Scan für Uploads | ERLEDIGT – CI-LAUF NACH LETZTEM FIX AUSSTEHEND | `src/server/services/malware-scan.ts` (ClamAV `clamscan`, fail-closed) in `finalizeAvatarUpload`/`finalizeUpload` hinter `MALWARE_SCAN_ENABLED`. War bereits einmal real in CI grün (`gh run view 29854798546`). Ein eigener Isolations-Versuch (paralleler Scan-Last mit einem separaten, seriellen Lauf begegnen) führte zu zwei echten, root-caused CI-Bugs (D-079: Server-Neustart nach Zwischen-Reset fehlte; D-080: ein vermeintlicher Dev/Prod-Unterschied entpuppte sich als eigener Testfehler – vergessenes `MALWARE_SCAN_ENABLED=true` beim manuellen Serverstart). Nach beiden Korrekturen zweimal in Folge lokal exakt CI-äquivalent reproduziert, mit expliziter Umgebungsvariablen-Bestätigung: 8/8 grün, < 51s | Nach Push echten CI-Lauf bestätigen; Produktions-Dauerlösung (`clamd`-Daemon oder Cloud-AV) bleibt an Toms Hosting-Wahl gebunden (siehe A3) |
...
- **ERLEDIGT UND VERIFIZIERT (Ergänzung):** Privacy-Dateninventur inkl. Apple-App-Privacy-/Google-Data-Safety-/Health-Mapping (`docs/PRIVACY_SECURITY.md` Abschnitt 1a), App-Icon/Splash aus eigener Marke, iOS-Fotozugriffstext, Store-Texte (Entwurf), Health-Check-Route + Deployment-Vorbereitung (`docs/DEPLOYMENT.md`).
